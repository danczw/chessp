...
class Board:
    ...
    def move_piece(
        self, startcoords: tuple, endcoords: tuple, white_turn: bool
    ) -> bool:
        ...
        if (moved_piece.color == "white" and not white_turn) or (
            moved_piece.color == "black" and white_turn
        ):
            print("Not your turn")
            return False
        ...
        if moved_piece.name_short == "K" and abs(startcoords[1] - endcoords[1]) == 2:
        ...
        # check for promotion
        if moved_piece.name_short == "P" and (endcoords[0] == 0 or endcoords[0] == 7):
    ...
    def _get_coords_between(self, startcoords: tuple, endcoords: tuple) -> list[tuple]:
        ...
        # diagonal move
            ...
            for row, col in zip(range(row_start, row_end), range(col_start, col_end)):
                coords_between.append((row, col))

        return coords_between

    def _castling(
        self, startcoords: tuple[int, int], endcoords: tuple[int, int]
    ) -> bool:
        # get rook
        if startcoords[1] < endcoords[1]:
    ...
    def _promotion(self, endcoords: tuple[int, int], moved_piece: Piece) -> Piece:
        pieces_dict = {"Queen": Queen, "Rook": Rook, "Bishop": Bishop, "Knight": Knight}
        ...
        new_piece_name = [
            k for k in pieces_dict.keys() if k.startswith(new_piece_name_short)
        ][0]

        # return promoted piece
        return pieces_dict[new_piece_name](
            coords=endcoords, color=moved_piece.color, name=new_piece_name
        )
