...
def test_king():
    """Test the king class
    """
    king = King(color='white', position=[2, 2])
...
def test_queen():
    ...
    """
    queen = Queen(color='black', position=[2, 2])
...
def test_rook():
    """Test the rook class
    """
    rook = Rook(color='white', position=[2, 2])
...
def test_bishop():
    ...
    """
    # test black bishop
    bishop_w = Bishop(color='white', position=[2, 2])
    ...
    # test white bishop
    bishop_b = Bishop(color='black', position=[3, 2])
...
def test_knight():
    ...
    """
    knight = Knight(color='white', position=[2, 2])
...
def test_pawn():
    ...
    """
    # test white pawn
    pawn_w = Pawn(color='white', position=[2, 2])
    
    assert pawn_w.get_color() == 'white'
    assert pawn_w.get_position() == [2, 2]
    
    assert pawn_w.move([0, 1]) == False
    assert pawn_w.move([2, 2]) == False
    assert pawn_w.move([2, 3]) == True
    assert pawn_w.move([2, 4]) == True
    assert pawn_w.move([3, 4]) == False
    
    assert pawn_w.get_position() == [2, 4]
    
    # test black pawn
    pawn_b = Pawn(color='black', position=[6, 6])
    
    assert pawn_b.get_color() == 'black'
    assert pawn_b.get_position() == [6, 6]
    
    assert pawn_b.move([0, 1]) == False
    assert pawn_b.move([6, 6]) == False
    assert pawn_b.move([6, 5]) == True
    assert pawn_b.move([6, 4]) == True
    assert pawn_b.move([5, 4]) == False
    assert pawn_b.move([6, 3]) == True
    
    assert pawn_b.get_position() == [6, 3]
